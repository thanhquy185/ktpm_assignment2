...
public class UserController {
        ...
        public ResponseEntity<?> handleLoginError() {
                List<User> users = this.userService.getUserByUsernameError();

                RestResponse<List<User>> restResponse = RestResponse.<List<User>>builder()
                                .status(HttpStatus.OK.value())
                                .message("Get user by username sql injection is successful!")
                                .data(users)
                                .build();
